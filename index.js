...
$(function() {
  $('.Candidates').on('touchstart', function() {
    touchStartTimestamp = Date.now();
  });

  $('.Candidates').on('touchend', function() {
    const touchEndTimestamp = Date.now();
    const touchDuration = touchEndTimestamp - touchStartTimestamp;

    if (touchDuration < 500) {
      alert('Короткий дотик');
      // Тут виконайте дії для короткого дотику
    } else {
      alert('Довгий дотик');
      // Тут виконайте дії для довгого дотику
    }
  });
